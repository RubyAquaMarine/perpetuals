...
import { ARBITRUM, ARBITRUM_TESTNET, TELOS_TESTNET, AVALANCHE, getConstant, getHighExecutionFee } from "config/chains";
...
export function useStakedGmxSupply(library, active) {
  ...
  const gmxAddressAvax = getContract(TELOS_TESTNET, "GMX");
  const stakedGmxTrackerAddressAvax = getContract(TELOS_TESTNET, "StakedGmxTracker");

  const { data: avaxData, mutate: avaxMutate } = useSWR(
    [`StakeV2:stakedGmxSupply:${active}`, TELOS_TESTNET, gmxAddressAvax, "balanceOf", stakedGmxTrackerAddressAvax],
    {
...
export function useTotalGmxSupply() {
  const gmxSupplyUrlArbitrum = getServerUrl(TELOS_TESTNET, "/gmx_supply");
...
export function useTotalGmxStaked() {
  ...
  const { data: stakedGmxSupplyAvax, mutate: updateStakedGmxSupplyAvax } = useSWR<BigNumber>(
    [
      `StakeV2:stakedGmxSupply:${TELOS_TESTNET}`,
      TELOS_TESTNET,
      getContract(TELOS_TESTNET, "GMX"),
      "balanceOf",
  ...
  if (stakedGmxSupplyArbitrum && stakedGmxSupplyAvax) {
    let total = bigNumberify(stakedGmxSupplyAvax); // bigNumberify(stakedGmxSupplyArbitrum)!.add(stakedGmxSupplyAvax);
...
export function useTotalGmxInLiquidity() {
  ...
  const { data: gmxInLiquidityOnAvax, mutate: mutateGMXInLiquidityOnAvax } = useSWR<any>(
    [`StakeV2:gmxInLiquidity:${TELOS_TESTNET}`, TELOS_TESTNET, getContract(TELOS_TESTNET, "GMX"), "balanceOf", poolAddressAvax],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
...
function useGmxPriceFromAvalanche() {
  ...
  const vaultAddress = getContract(TELOS_TESTNET, "Vault");
  const avaxAddress = getTokenBySymbol(TELOS_TESTNET, "WTLOS").address;

  const { data: avaxPrice, mutate: updateAvaxPrice } = useSWR(
    [`StakeV2:avaxPrice`, TELOS_TESTNET, vaultAddress, "getMinPrice", avaxAddress],
    {
...
function useGmxPriceFromArbitrum(library, active) {
  ...
  const vaultAddress = getContract(TELOS_TESTNET, "Vault");
  const ethAddress = getTokenBySymbol(TELOS_TESTNET, "WTLOS").address;
  const { data: ethPrice, mutate: updateEthPrice } = useSWR<BigNumber>(
    [`StakeV2:ethPrice:${active}`, TELOS_TESTNET, vaultAddress, "getMinPrice", ethAddress],
    {
